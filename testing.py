...
def training(
    ...
    try:
        ...
        '''
        data_AI = data[feature_cols].copy() # Get data base on inputs and outputs
        # dataAI = dataAI[:5000]
        ...
        for data_set in data_sets:
            ...
            for each_group in data_set:
                scaled_group=scaler.fit_transform(each_group)
        ...
        x_train_reshaped, combined_y_train = prepare_training_data(training_datas_scaled, WS,max_length_token_data, feature_count)
        x_test_reshaped, combined_y_test = prepare_training_data(testing_datas_scaled, WS,max_length_token_data, feature_count)
        '''
...
def define_model(interval:str, max_length_token_data, WS, feature_count, is_scale:bool=True):
    ...
    modelPath = f"src/models/{interval}/model_test.h5"
    ...
    if os.path.isfile(modelPath):
        ...
        model.add(Dense(units = max_length_token_data, activation = 'sigmoid'))  # Add new output layer
        ...
        model.add(Dense(units = max_length_token_data, activation = 'sigmoid'if is_scale == True else None))
...
def data_process(data, feature_cols):
